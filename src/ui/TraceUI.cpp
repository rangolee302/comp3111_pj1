...
void TraceUI::cb_load_BG(Fl_Menu_* o, void* v)
{
	...
	char* newfile = fl_file_chooser("Open BG?", "*.jpeg", NULL);

	if (newfile != NULL) {
		char buf[256];

		if (pUI->raytracer->loadBG(newfile)) {
...
void TraceUI::cb_adativeSlides(Fl_Widget* o, void* v) 
{
	TraceUI* pUI = (TraceUI*)(o->user_data());

	pUI->m_nAdaptive = float(((Fl_Slider *)o)->value());
}
...
void TraceUI::cb_superSampleSlides(Fl_Widget* o, void* v)
{
	...
	pUI->superSamplingLevel = int(((Fl_Slider *)o)->value());
}
...
int TraceUI::getSuperSampleLevel()
{
	return superSamplingLevel;
}

float TraceUI::getAdaptive()
{
	return m_nAdaptive;
}
...
TraceUI::TraceUI() {
	...
	m_mainWindow = new Fl_Window(100, 40, 320, 200, "Ray <Not Loaded>");
		...
		// install slider adative
		m_adaptiveSlider = new Fl_Value_Slider(10, 80, 180, 20, "Adative Thresh");
		...
		m_adaptiveSlider->minimum(0.0);
		m_adaptiveSlider->maximum(1.0);
		m_adaptiveSlider->step(0.1);
		m_adaptiveSlider->value(m_nAdaptive);
		m_adaptiveSlider->align(FL_ALIGN_RIGHT);
		m_adaptiveSlider->callback(cb_adativeSlides);

		// install slider adative
		m_superSamplingSlider = new Fl_Value_Slider(10, 105, 180, 20, "Super Sample Level");
		...
		m_superSamplingSlider->value(superSamplingLevel);
		m_superSamplingSlider->align(FL_ALIGN_RIGHT);
		m_superSamplingSlider->callback(cb_superSampleSlides);
